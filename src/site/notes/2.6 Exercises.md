---
{"dg-publish":true,"permalink":"/2-6-exercises/","created":"2023-08-24T17:05:25.544+02:00","updated":"2023-10-24T15:37:40.953+02:00"}
---
...
$$
\begin{split}
1) &\qquad f\colon\mathbb{R}^{2}\rightarrow\mathbb{R},\qquad f(x,y)=x\mathrm{e}^{x^{2} - y^{2}} \\ & \\
2) & \qquad f\colon\mathbb{R}^{2}\rightarrow\mathbb{R},\qquad f(x,y)=\left\{\begin{matrix} \frac{xy}{\sqrt{x^{2}+y^{2}}} & (x,y)\neq (0,0) \\ 0 & (x,y)=(0,0) \end{matrix}\right.\\ & \\
3) & \qquad f\colon\mathbb{R}^{2}\rightarrow\mathbb{R},\qquad f(x,y)=\left\{\begin{matrix} \frac{x^{2}y}{x^{4}+y^{2} } & (x,y)\neq (0,0) \\ 0 & (x,y)=(0,0) \end{matrix}\right.\\ & \\
4) &\qquad f\colon\mathbb{R}^{3}\rightarrow\mathbb{R}^{3},\qquad f(x,y,z)=\left(\begin{matrix}3x +5y \\ 2y + 6z^{2} \\x^{3} - y^{5} +z^{7}   \end{matrix}\right)\\ & \\
5) &\qquad f\colon\mathbb{R}^{3}\rightarrow\mathbb{R}^{3},\qquad f(x,y,z)=\left(\begin{matrix}\ln(x^{2}+y^{2}+1 ) \\ 2y\,\cos(z^{2}x) \\ \arctan\left(x^{4} +y^{4} + \sqrt{z^{2} +1}\right)   \end{matrix}\right)\\ & \\
\end{split}
$$
...
$$
\begin{split}
1) &\qquad f(x,y)=xy, \quad g(t)=(\mathrm{e}^{t},\cos(t))\\ & \\
2) &\qquad f(x,y)=(x^{2} + y^{2})\ln\left(\sqrt{x^{2} + y^{2}}\right), \quad g(t)=(\mathrm{e}^{t},\mathrm{e}^{-t}) \\ & \\
3) &\qquad f(u,v)=(\tan(u-1) - \mathrm{e}^{v}, u^{2} - v^{2}),\quad g(x,y)=(\mathrm{e}^{x-y},x-y),\quad(x,y)=(1,1) \\ & \\
4) &\qquad f(u, v, w) = (\mathrm{e}^{u-w}, \cos(v + u) +
\sin(u + v + w)),\quad g(x, y) = (\mathrm{e}^{x} , \cos( y -x), \mathrm{e}^{-y} ),  \quad (x,y)=(0, 0).
\end{split}
$$
...
4) Consider the funcrion $f\colon\mathbb{R}^{2}\rightarrow\mathbb{R}$ given by
...
- Consider the function $g\colon\mathbb{R}\rightarrow\mathbb{R}^{2}$ given by $g(t)=(at,bt)$ with $a,b\in\mathbb{R}$ and compute the derivative of $f\circ g$ at $t=0$ both directly and using the chain rule. What can we conclude?
{ #fbb3a9}

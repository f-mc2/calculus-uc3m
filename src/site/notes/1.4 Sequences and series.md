---
{"dg-publish":true,"permalink":"/1-4-sequences-and-series/","created":"2023-06-25T19:27:01.186+02:00","updated":"2023-09-19T22:31:51.141+02:00"}
---
...
>[!defn] Definition
>A numerical sequence is a function $a\colon \mathbb{N}\rightarrow\mathbb{R}$. Often, a numerical sequence is also written as $\{a_{n}\}_{n\in\mathbb{N}}$ (or just $\{a_{n}\}$, depending on the degree of laziness) where $a_{n}=a(n)$. 
...
>$$
>which requires the first two terms to be defined separately from the others. 
...
> $$ 
> and say that the **numerical series** determined by $\{a_{n}\}$  is **convergent**. The series $S$ is said to be **absolutely convergent** if the series 
> $$
...
>s_{k}= k + 1,
>$$
>which means the series diverges because of [[1.4 Sequences and series#^ba1349\|the necessary condition for convergence]]. When $x=-1$, it is
>$$ s_{k}=\left\{\begin{matrix}1 & \mbox{ if $k$ is even}\\ \\ 0 & \mbox{ if $k$ is odd},\end{matrix}\right.$$ 
>which means the series does not converge. On the other hand, when $x\neq \pm1$, it holds 
>$$
...
>Let $S_{a}$ be the series determined by $\{a_{n}\}$, with $0\leq a_{n}$ for all $n\in\mathbb{N}$ and $$ \lim_{n\rightarrow+\infty}\frac{a_{n+1}}{a_{n}}=L. $$
>Then:
>1. $S_{a}$ converges if $0\leq L<1$;
>2. $S_{a}$ diverges if $L>1$;
>3. if $L=1$ then the test is inconclusive.

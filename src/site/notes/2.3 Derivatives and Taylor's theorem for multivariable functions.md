---
{"dg-publish":true,"permalink":"/2-3-derivatives-and-taylor-s-theorem-for-multivariable-functions/","created":"2023-08-24T17:02:57.002+02:00","updated":"2023-10-24T16:54:41.663+02:00"}
---
...
>$$
>f(x^{1},\cdots , x^{n})\equiv f(\mathbf{x})=\left(\begin{matrix} \;f^{1}(\mathbf{x}) \;\\ \; f^{2}(\mathbf{x}) \; \\ \;\vdots \; \\ \; f^{n}(\mathbf{x}) \;\end{matrix}\right)
>$$
...
>$$
>where the limit in the right-hand-side is taken according to the definition given [[1.2 Limits of scalar functions\|here]].
...
>Given $f\colon D\subseteq\mathbb{R}^{k}\rightarrow\mathbb{R}^{n}$, to compute the partial derivative $\left.\frac{\partial f^{i}}{\partial x^{j}}\right|_{\mathbf{x}_{0}}$, we first compute the derivative of $f^{i}$ with respect to $x^{j}$ using the rules we learned for [[1.3 Derivatives of scalar functions\|scalar functions]] by considering all variables except $x^{j}$ to be constants. If the result is well-defined at $\mathbf{x}_{0}\in D$ then that is the partial derivative we were looking for. If the result is not well-defined at $\mathbf{x}_{0}\in D$, then we must use the original definition of partial derivative in terms of limits.
...
>The function $f$ is said to be $C^{1}$ in $A\subseteq D$ if it is differentiable in $A$ and all its partial derivatives $\frac{\partial f^{i}}{\partial x^{j}}$ are [[5.2 Limits and continuity of multivariable vector functions\|continuous]] in $A$.

>[!rem] Remark: tangent hyperplane
>When $n=1$, fixing both $f$ and $\mathbf{x}_{0}$, the object $f(\mathbf{x}_{0})+\mathbf{D}f(\mathbf{x}_{0})(\mathbf{x} - \mathbf{x}_{0})$ can be thought of as a multivariable scalar function which is linear in $\mathbf{x}$, that is, an hyperplane in $\mathbb{R}^{k}$. This hyperplane is called **tangent** to $f$ at $\mathbf{x}_{0}$, and when $f$ is differentiable at $\mathbf{x}_{0}$ it follows that the tangent hyperplane is a good approximation to $f$ around $\mathbf{x}_{0}$.
...
>$$
>
>which holds because of the very definition of [[5.2 Limits and continuity of multivariable vector functions\|limits of multivariable scalar functions]].
...
Given $f\colon D\subseteq \mathbb{R}^{n}\rightarrow \mathbb{R}$, [[2.3 Derivatives and Taylor's theorem for multivariable functions\|we know]] that there are $n$ partial derivatives, say $\frac{\partial f}{\partial x^{j}}$ with $j=1,...,n$. These partial derivatives can be arranged into a vector whose role in the search for local extrema can not be overemphasized.
...
>If all the possible second order partial derivatives of the function $f\colon D\subseteq \mathbb{R}^{n}\rightarrow \mathbb{R}$ exist and are continuous on $A\subseteq D$, then $f$ is said to be $C^{2}$ on $A$.

>[!defn] Proposition: equality of mixed partial derivatives
>If $f\colon D\subseteq \mathbb{R}^{n}\rightarrow \mathbb{R}$ is $C^{2}$ on $A\subseteq D$ then 
>$$
...
Once we have the matrix of second partial derivatives, we can take the partial derivatives of each element in the matrix, and obtain an object (which is neither a vector nor a matrix) encoding information on third order derivatives.

>[!defn] Definition: $C^{3}$ functions
>If all the possible third order partial derivatives of the function $f\colon D\subseteq \mathbb{R}^{n}\rightarrow \mathbb{R}$ exist and are continuous on $A\subseteq D$, then $f$ is said to be $C^{3}$ on $A$.

Of course, nothing prevents us to investigate fourth order and higher order partial derivatives. However, for $f\colon D\subseteq \mathbb{R}^{n}\rightarrow\mathbb{R}$, we get $n^{k}$ partial derivatives of order $k$, and thus number of computations becomes quickly unmanageable.

# Second order Taylor's theorem

Just as in the [[1.5 Power series, trascendental functions, and Taylor series#^113a5b\|one-variable case]], derivatives help in getting a good approximation of a given function. This approximation is ruled by Taylor's theorem.

>[!important] Theorem: second order Taylor's theorem
>Let $f\colon D\subseteq \mathbb{R}^{n}\rightarrow\mathbb{R}$ be of class $C^{3}$. $(k+1)$-times differentiable in an open interval $I$ containing $x_{0}$. Then, we have
>$$

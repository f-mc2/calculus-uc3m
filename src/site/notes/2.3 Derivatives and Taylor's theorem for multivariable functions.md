---
{"dg-publish":true,"permalink":"/2-3-derivatives-and-taylor-s-theorem-for-multivariable-functions/","created":"2023-08-24T17:02:57.002+02:00","updated":"2023-11-07T22:56:08.617+01:00"}
---
...
>For a given multivariable scalar function $f$ and a point $\mathbf{x}_{0}\in D$ at which partial derivatives exist, the object $f(\mathbf{x}_{0})+\mathbf{D}f(\mathbf{x}_{0})(\mathbf{x} - \mathbf{x}_{0})$ is a linear function of $\mathbf{x}$. Therefore, it defines an hyperplane in $\mathbb{R}^{k}$ called **tangent** to $f$ at $\mathbf{x}_{0}$. When $f$ is differentiable at $\mathbf{x}_{0}$, it follows that the tangent hyperplane is a good approximation to $f$ around $\mathbf{x}_{0}$.

---
{"dg-publish":true,"permalink":"/1-7-exercises/","created":"2023-07-02T17:05:44.464+02:00","updated":"2023-09-22T15:04:27.670+02:00"}
---
...
1) Prove the following limits 
	...
	>2) For $|x|< 1$, the sequence $a_{n}=x^{n}$ has limit either $0$. Therefore, exploiting the result of point 1) together with the algebraic manipulations of limits of sequences, the desired result follows. When $x=1$, the sequence $a_{n}=1^n$ has limit $1$. Reasoning as before, the result follows. When $x=-1$,  the sequence $a_{n}=(-1)^n$ has no limit. However, $\vert\frac{(-1)^n}{n!}\vert=\vert \frac{1}{n!}\vert$, and we know that 
	>		$$ \lim_{n\rightarrow+\infty}\frac{1}{n!}=0 $$
	>			because of the result of point 1) and the algebraic manipulations of limits. Therefore, we may exploit the [[1.4 Sequences and series#^0cefdc\|very definition of limit]] and immediately conclude the desired result.
...
2) Check the following limits:
	$$\begin{split} 	1) \quad & \lim_{x\rightarrow 0}\, \frac{\tan(x^{2})+2x}{x+x^{2}}= 2\\ & \\	2) \quad & \lim_{x\rightarrow 0}\,\frac{\mathrm{e}^{x} - x - \cos(x)}{\sin(x^{2})} = 1 \\ & \\	3) \quad & \lim_{x\rightarrow +\infty}\,\left(1 + \frac{1}{x}\right)^{x}= \mathrm{e}  \\ & \\	4) \quad & \lim_{x\rightarrow +\infty}\, \frac{\left(1 + \frac{1}{x}\right)^{x^{2}}}{\mathrm{e}^{x}} = \mathrm{e}^{-\frac{1}{2}}\\ & \\	5) \quad &  	\end{split}$$
	>	[!note]- Solution
	>	1) We first write
...
1) Study the global extrema of the following functions:
	$$	\begin{split}	1) \quad & f(x)\,=\; x + \frac{1}{x} \\ & \\	2) \quad & f(x)\,=\; \frac{x^{2}}{1+x} \\ & \\	3) \quad & f(x)\,=\;  x^{2}\,\left(2+x\right)^{\frac{1}{3}}\\ & \\	4) \quad & f(x)\,=\; |x - 3| + |2x + 1|  \\ & \\	5) \quad & f(x)\,=\;  x + \cos(2x) \\ & \\	6) \quad & f(x)\,=\;  \left\{\begin{matrix} 2 - 2x -x^{2} & \quad -2\leq x\leq 0 \\ |x-2| & \quad  0<x<3 \\ \frac{1}{3}\left(x-2\right)^{2} & \quad  3\leq x \leq 4 \end{matrix}\right.\\ & \\	7) \quad & f(x)\,=\; \left\{\begin{matrix} x^{2} +1 & \quad  -2\leq x < 1 \\ 5 + 2x -x^{2}& \quad  -1\leq x\leq 3 \\ x-1 & \quad  3< x < 6 \end{matrix}\right.  \\ & \\	8) \quad & f(x)\,=\;  \left\{\begin{matrix} \frac{3-x^{2}}{2} & \quad  x < 1 \\ \frac{1}{x} & \quad  -1\leq x    \end{matrix}\right.\\ & \\	9) \quad & f(x)\,=\; 1 - (x-1)^{\frac{1}{3}} \\ & \\	10) \quad & f(x)\,=\; \sqrt{x} - \frac{1}{\sqrt{x}} \\ & \\	\end{split}	$$
	
	>[!note]- Solution
	>1) The maximal domain of the function is $D=\mathbb{R}-\{0\}$, and it is smooth in $D$ because it is the sum of two smooth functions in $D$. In particular, it is continuous at each point $x\neq 0$. The behavior at $0$ is determined uniquely by $\frac{1}{x}$. Therefore, building on previous exercises, we know the limit for $x\rightarrow0^{\pm}$ is $\pm\infty$.  The limits at $\pm\infty$ are determined by $x$, and we know they are $\pm\infty$ because of previous exercises. From this, we already conclude that there will be no global maxima nor minima. Concerning local extrema, we compute the derivative and look for critical points
	>		$$ f'(x)= 1 -\frac{2}{x^{2}}\;\Longrightarrow\; f'(x)=0 \;\Longrightarrow x^{2}=2\;\Longrightarrow x=\pm\sqrt{2}.$$
	>		There are no other critical points because the derivative exists at each point in the maximal domain of the function. The condition $f'(x)>0$ implies $|x|>0$, and thus we see that $x=\sqrt{2}$ is a local minimum, while $x=-\sqrt{2}$ is a local maxima.
	>	2) The maximal domain of the function is $D=\mathbb{R}-\{-1\}$, and the function is smooth in $D$ because it is the quotient of two smooth functions in $D$. Remembering what previous exercises taught us about $\frac{1}{x}$, we conclude $f(x)$ tends to $\pm\infty$ when $x\rightarrow -1^{\pm}$. Then, we have

---
{"dg-publish":true,"permalink":"/1-7-exercises/","created":"2023-07-02T17:05:44.464+02:00","updated":"2023-08-30T11:09:40.491+02:00"}
---
...
1) Compute
	$$ \lim_{x\rightarrow x_{0}} f(x)= f(x_{0}) $$for $f(x)=c$, $f(x)=x$, and $f(x)=x^{2}$.
...
3)  Use the [[1.2 Limits of scalar functions#Improper limits\|definition]] to check the following limits: 
	...
	6)   \quad & \lim_{x\rightarrow \pm\infty} \;\frac{1}{x^{n}} =0 \\& \\ \end{split} $$
	>[!note]- Solution 
...
5) Check the following limits: 
	...
	1) \quad &  \lim_{x\rightarrow 0}\, \frac{x}{|x|}=\nexists \\ & \\
	2) \quad &  \lim_{x\rightarrow 0}\, \frac{|x|}{ x }=\nexists \\ & \\
	3) \quad &  \lim_{x\rightarrow 0}\, f(x) =\nexists \quad \mbox{ with } f(x)= \left\{\begin{matrix} 1-x^2 & \mbox{ if } x<0 \\ x^3 & \mbox{ if } x\geq 0\end{matrix}\right.  \\ & \\
	4) \quad &  \lim_{x\rightarrow 1^{\pm}}\, \frac{x^{2}-1}{ x^{2} -2x + 1 }= \pm\infty\\ & \\
	5) \quad &  \lim_{x\rightarrow 2^{+}}\, \frac{\sqrt{x^{2} -4}}{x-2} = +\infty \end{split} $$
	>[!note]- Solution
	...
	>5) When $x=1$, both numerator and denominator vanish, and we get an indeterminate form. However, the fact that both polynomials have a common root ($x=1$) suggests we [factorize them](https://en.wikipedia.org/wiki/Polynomial#Factoring). In particular, we have $x^{2}-1=(x-1)(x+1)$, and $x^{2}-2x +1=(x-1)(x-1)$. Consequently, we have $$ \frac{x^{2}-1}{x^{2}-2x+1}=\frac{x+1}{x-1}=1+ \frac{2}{x-1}.$$ To check the right limit, we need to show that, for every $K>0$, there is $\delta>0$ such that  $0<x-1<\delta$ implies $|1+\frac{2}{x-1}|>K$. Now, $\frac{2}{x-1}>0$ when $0<x-1<\delta$, so that $1+\frac{2}{x-1}>1+ \frac{2}{\delta}>K$ provided $\delta<\frac{2}{K-1}$. The left limit is found proceeding in analogy.
	>6) Following the reasoning in point 5), we obtain $$ \frac{\sqrt{x^{2}-4}}{x-2}=\sqrt{1+\frac{4}{x-2}}.$$ Then, to check the right limit, we need to show that, for every $K>0$, there is $\delta>0$ such that  $0<x-2<\delta$ implies $\sqrt{1+\frac{4}{x-2}}>K$. Now, $\frac{4}{x-2}>0$ when $0<x-2<\delta$, so that $1+\frac{4}{x-2}>1+ \frac{4}{\delta}>K^{2}$ provided $\delta<\frac{4}{K^{2}-1}$
...
5) \quad & \lim_{n\rightarrow +\infty}\,\left(1+\frac{1}{n}\right)^{n}=e   
\end{split}
$$
...
3) Determine the behavior of the arithmetic-geometric series defined by $a_{n}=nx^{n}$.


1) Check the following equalities:

$$
\begin{split}
1) &\qquad \sum_{n=1}^{+\infty}a_{n}\,=\; \sum_{n=1}^{+\infty}\,\frac{1}{2n(n+1)}\\ & \\
2)  &\qquad \sum_{n=0}^{+\infty}a_{n}\,=\; \sum_{n=0}^{+\infty}\;\frac{1}{10^{n}} \\ & \\
3)  &\qquad \sum_{n=3}^{+\infty}a_{n}\,=\; \sum_{n=3}^{+\infty}\;\frac{1}{n^{2} -n} \\ & \\
4)  &\qquad \sum_{n=0}^{+\infty}a_{n}\,=\; \sum_{n=0}^{+\infty}\;\frac{1}{(n+1)(n+3)} \\ & \\
5)  &\qquad \sum_{n=0}^{+\infty}a_{n}\,=\; \sum_{n=0}^{+\infty}\;\frac{2^{n+3}}{3^{n}}
\end{split}
$$

---
{"dg-publish":true,"permalink":"/1-7-exercises/","created":"2023-07-02T17:05:44.464+02:00","updated":"2023-09-05T10:51:28.823+02:00"}
---
...
1) Compute the derivatives, at all points in the maximal domain at which it makes sense, of the scalar functions determined by:
	$$ \begin{split} 1) \quad &  f(x) =\ln(x-a) \\ & \\ 2) \quad & f(x) =\ln(|x|) \\ & \\ 3) \quad & f(x)= x^{\alpha}   \\ & \\
	4) \quad & f(x) = \arcsin(x) \\ & \\ 	5) \quad & f(x) = \arccos(x) \\ & \\ 	6) \quad & f(x) = \tan(x)  \\ & \\ 	7) \quad & f(x) =\cot(x) \\ & \\ 	8) \quad & f(x) = \arctan(x) \\ & \\ 	9) \quad & f(x) =  x^{x} \\ & \\ 	10) \quad & f(x) = \sqrt{\tan(x)} \\ & \\ 	11) \quad & f(x) = \arctan\left(x^{3} - e^{x^{2}}\right) \\ & \\ 	12) \quad &  f(x)=(g(x))^{h(x)} \\ & \\	13) \quad & f(x)= \arctan\left(\frac{g(x)}{h(x)}\right)  \end{split} $$
		>[!note]- Solution
...
2) Given the limit
	$$ \lim_{x\rightarrow 0}\, \frac{\sin(x)}{x}= 1, $$
		Can we use L'Hôpital's rule to compute it?

3) Using L'Hôpital's rule, Taylor's approximation, or whichever magic you see fit, check the following limits:
...
Study the global extrema of the following functions:

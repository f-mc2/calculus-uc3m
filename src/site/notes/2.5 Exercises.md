---
{"dg-publish":true,"permalink":"/2-5-exercises/","created":"2024-10-29T20:31:36.281+01:00","updated":"2024-11-04T13:30:37.375+01:00"}
---
...
3) Determine the points at which $(\mathbb{R}^{2},\mathbb{R}^{2},f,\mathbb{R})$ is continuous when
	...
	>5) It is best to write
	>	$$\frac{\sin(x-y)}{\mathrm{e}^{x}-\mathrm{e}^{y}}=\mathrm{e}^{-y}\,\frac{\sin(x-y)}{\mathrm{e}^{x-y}-1}.$$
	>	Then, we consider the limit
	>	$$\lim_{x\rightarrow c} \mathrm{e}^{-c}\,\frac{\sin(x-c)}{\mathrm{e}^{x-c}-1}=\lim_{z\rightarrow 0} \mathrm{e}^{-c}\,\frac{\sin(z)}{\mathrm{e}^{z}-1},$$
	>	where we have set $x-c=z$ and used [[1.2 Limits of scalar functions#^7ca684\|this proposition]]. To evaluate the previous limit, we use Taylor's approximation at first order around $z=0$, and obtain the limit $\mathrm{e}^{c}$. Therefore, whenever $x=y=c\neq 0$, the function can not be continuous. At the point $(0,0)$, however, the function is continuous, but the proof of its continuity is best done showing it is differentiable at that point. 
